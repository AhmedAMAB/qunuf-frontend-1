...
export default function Popup({ children, onClose, show, className, headerContent }: PopupProps) {
    ...
    return createPortal(
        ...
        >
            <div
                ref={popupRef}
                className={` max-h-[90vh] overflow-y-auto w-fit  md:min-w-[500px]  relative bg-white rounded-xl shadow-lg p-4 md:p-5 lg:p-6  space-y-2 transition-all duration-300 scale-100  ${className}`}
            >
                {/* Close Button */}
                ...
                <div className="flex ">
                    ...
                    {onClose && (<button
                        onClick={onClose}
                        className="text-gray-500 hover:text-gray-700 ms-auto ps-2"
                        aria-label="إغلاق"
                    >
